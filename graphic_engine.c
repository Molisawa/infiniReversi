    ...
    for (int i = 0; i < board->size; i++) {
        for (int j = 0; j < board->size; j++) {
            ...
            switch (board->state[i][j].pieceType) {
                case HELPER: {
                    vector = (Vector2) {(i) * screenFeatures->squareSize + screenFeatures->squareSize / 2, (j) * screenFeatures->squareSize + screenFeatures->squareSize / 2};
                    if (CheckCollisionPointCircle(mouse, vector, screenFeatures->squareSize / 2 - 5)) {
                        if (!clicked) {
                            DrawRectangle((i) * screenFeatures->squareSize + 1, (j) * screenFeatures->squareSize + 1, screenFeatures->squareSize - 2,
                                          screenFeatures->squareSize - 2, DARKGREEN);
                            DrawCircle((i) * screenFeatures->squareSize + screenFeatures->squareSize / 2, (j) * screenFeatures->squareSize + screenFeatures->squareSize / 2,
...
void DrawBoardGrid(Board *board, ScreenFeatures *screenFeatures) {
    for (int i = 0; i < board->size + 1; i++) {
        DrawLineV((Vector2) {screenFeatures->squareSize * i, 0}, (Vector2) {screenFeatures->squareSize * i, board->size * screenFeatures->squareSize}, BLACK);
    }

    for (int i = 0; i < board->size + 1; i++) {
        DrawLineV((Vector2) {0, screenFeatures->squareSize * i}, (Vector2) {board->size * screenFeatures->squareSize, screenFeatures->squareSize * i}, BLACK);
    }
}
...
void DrawBackground(Board *board, Menu menu, ScreenFeatures *screenFeatures, ScreenFlag *screen,
    ...
    DrawRectangle(board->size * screenFeatures->squareSize + 1, 0, screenFeatures->screenWidth - 1, screenFeatures->screenHeight, WHITE);
    DrawRectangle(board->size * screenFeatures->squareSize + 1, 0, screenFeatures->screenWidth - 1, screenFeatures->screenHeight, Fade(DARKGREEN, 0.5f));
...
void ShowFileSaver(Board *board, ScreenFeatures *screenFeatures, char *filename, int frameCounter, Vector2 mouse,
    ...
    Rectangle saveRect = (Rectangle) {screenFeatures->screenWidth / 2 - (saveWidth + cancelWidth + 40) / 2, screenFeatures->screenHeight / 2 + 50,
                                      saveWidth, 30};
    ...
    DrawRectangleLines(screenFeatures->screenWidth / 2 - width / 2, screenFeatures->screenHeight / 2 - 20, width, 40, BLACK);
    DrawText(filename, screenFeatures->screenWidth / 2 - width / 2 + 10, screenFeatures->screenHeight / 2 - 15, 30, BLACK);
    if (((frameCounter / 20) % 2) == 0)
        DrawText("_", screenFeatures->screenWidth / 2 - width / 2 + MeasureText(filename, 30) + 10, screenFeatures->screenHeight / 2 - 15, 30, BLACK);
...
void LoadFile(Board *board,ScreenFeatures *screenFeatures, ScreenFlag *screen, Slider *slider) {
    ...
    float barSize =
            ((float) (screenFeatures->screenHeight - 80) / (float) (50 * directory.NumberOfDirectories + 10)) * (screenFeatures->screenHeight - 70);
    ...
    Rectangle scrollRect = (Rectangle) {screenFeatures->screenWidth - 20,
                                        5 +
                                        fmin(fmax(0, slider->offset + slider->difference), screenFeatures->screenHeight - 70 - barSize),
                                        15,
                                        barSize};
    if (bar) {
    ...
    for (int i = 0; i < directory.NumberOfDirectories; i++) {
        Rectangle rec = (Rectangle) {10, i * 50 + 10 -
                                         (((50 * directory.NumberOfDirectories + 10) - screenFeatures->screenHeight + 60) * percent),
    ...
    DrawText("CANCEL", screenFeatures->screenWidth / 2 - MeasureText("CANCEL", 30) / 2 + cancelRect.x / 2, cancelRect.y + 10, 30,
             WHITE);
...
Menu getMenu(Board board, ScreenFeatures *screenFeatures) {
    ...
    return (Menu) {goBackButton, goFowardButton, saveGameButton, loadGameButton};
